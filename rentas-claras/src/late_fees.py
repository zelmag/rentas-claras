...
MAX_DAILY_PENALTY_DAYS = 7  # Maximum days that accrue daily penalty (days 3-9)
TERMINATION_WARNING_DAY = 8
MAX_TOTAL_PENALTY_MXN = Decimal(
    "1200.00"
)  # Maximum total penalty cap ($500 + 7*$100 = $1200)
...
def calculate_rentas_claras_balance(
    ...
    # Apply maximum penalty cap ($1200)
    if total_penalties > MAX_TOTAL_PENALTY_MXN:
        total_penalties = MAX_TOTAL_PENALTY_MXN
...
def _build_breakdown(
    ...
    if daily_penalties > 0:
        ...
        lines.append(
            f"📈 Recargo diario ({daily_days} días × $100): ${daily_penalties:,.2f}"
        )

    if already_paid > 0:
        lines.append(f"✅ Abonado: -${already_paid:,.2f}")

    lines.extend(["", f"💰 *TOTAL A PAGAR: ${total_due:,.2f} MXN*"])
...
def calculate_tenant_utilities(
    ...
    if (
        property_config.property_type == PropertyType.SHARED_METER
        and tenants_on_meter > 1
    ):
...
def calculate_property_subtotal(
    property_config: PropertyConfig, tenant_results: list[LateFeeResult]
) -> PropertySubtotal:
    """
    ...
    for result in tenant_results:
        ...
        if result.status == PaymentStatus.PAID:
            ...
            PaymentStatus.LATE_INITIAL,
            PaymentStatus.LATE_ACCUMULATING,
        ):
            late_count += 1
...
def calculate_grand_total(property_subtotals: list[PropertySubtotal]) -> GrandTotal:
    """
...
def run_late_fee_demo():
    ...
    for day, description in test_cases:
        ...
        print(
            f"   Base: ${result.base_rent:,.2f} + Utilities: ${result.utilities:,.2f}"
        )
    ...
    print(
        f"   Full amount would be: ${Decimal(str(base_rent)) + Decimal(str(utilities)) + result.total_penalties:,.2f}"
    )
    ...
    tenant_share = calculate_tenant_utilities(
        total_utility_bill=total_bill, property_config=matehuala_config
    )
...
def run_subtotals_demo():
    ...
    ensenada_subtotal = calculate_property_subtotal(
        PROPERTIES["ensenada"], ensenada_results
    )
    huichapan_subtotal = calculate_property_subtotal(
        PROPERTIES["huichapan"], huichapan_results
    )
    matehuala_subtotal = calculate_property_subtotal(
        PROPERTIES["matehuala_a"], matehuala_results
    )

    # Calculate grand total
    grand_total = calculate_grand_total(
        [ensenada_subtotal, huichapan_subtotal, matehuala_subtotal]
    )
