...
def test_excel_tables():
    ...
    excel_path = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "Inquilinos.xlsx"
    )
    ...
    for sheet_name in wb.sheetnames:
        ...
        if ws.tables:
            for table in ws.tables.values():
                ...
                print(
                    f"   Style: {table.tableStyleInfo.name if table.tableStyleInfo else 'None'}"
                )
                print(
                    f"   Show Row Stripes: {table.tableStyleInfo.showRowStripes if table.tableStyleInfo else 'N/A'}"
                )
        else:
            print("   NO TABLES IN THIS SHEET")
